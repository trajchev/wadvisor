...
export class TotalsChartComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.matchService.getTotals(this.key, this.id, 'totals').subscribe(totals => {
      this.drawGraph(totals);
    });
  }

  drawGraph(data): void {
    const margin = { left: 40, right: 20, bottom: 40, top: 100 };
    const svgWidth = document.querySelector('.totals-chart').clientWidth;
    const svgHeight = document.querySelector('.totals-chart').clientHeight;
    ...
    const color = d3.scaleOrdinal()
      .range(["#54C479", "#444444", "#248E38", "#444444"]);
    ...
    this.svg = d3.select('.totals-chart')
      .append('svg')
      .attr('width', svgWidth)
  ...
  loadGraphData(data, x, xg, y, keys, color, tip) {
    this.graph
      ...
      .join('rect')
      .attr('x', d => d.key === keys[1] || d.key === keys[3] ? xg(d.key) - xg.bandwidth() : xg(d.key) + xg.bandwidth() - 10)
      .attr('y', d => y(d.value))
      .attr('width', d => d.key === keys[1] || d.key === keys[3] ? xg.bandwidth() * 2 : 20)
      .attr('rx', d => d.key === keys[1] || d.key === keys[3] ? 0 : 10)
      .attr('ry', d => d.key === keys[1] || d.key === keys[3] ? 0 : 10)
      .on('mouseover', tip.show)
      .on('mouseout', tip.hide)
      .transition()
      .duration(this.duration)
      .attr('height', d => d.key === keys[1] || d.key === keys[3] ? 4 : 20)
  ...
  legend(svgWidth, color, keys) {
    ...
    legend
      ...
      .attr('x', -18)
      .attr('width', 18)
      .attr('height', d => d === keys[1] || d === keys[3] ? 4 : 18)
      .attr('rx', d => d === keys[1] || d === keys[3] ? 0 : 10)
      .attr('ry', d => d === keys[1] || d === keys[3] ? 0 : 10)
      .attr('transform', d => `translate(0, ${d === keys[1] || d === keys[3] ? 7 : 0})`)
      .attr('fill', color);
